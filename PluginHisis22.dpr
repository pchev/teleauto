...
begin
   ...
   if Good then Good:=WritePar(Adress,$96);
   ...
   if Good then Good:=WritePar(Adress,1);
   ...
   // For Hisis 22 the begin of the exposure is after the emptying delay !
   TimeInit:=Time;
...
@bl5:
 ...
 for k:=1 to Sx do TabImgInt^[1]^[j]^[k]:=Line[k]; // Every pixel is tranfered in the TabImgInt
end;
