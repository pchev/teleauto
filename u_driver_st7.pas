...
{	Function Prototypes
	...
	commands are handled by the ParDrvCommand() routine
...
//const Sbig32 = 'Sbig32.dll';
//function ParDrvCommandSbig(command:Word;var Params,Results):Word;
//         cdecl; external Sbig32 name '_ParDrvCommand';

function ParDrvCommand(Command:Word;var Params,Results):Word; cdecl;
         external 'SBIG32M.DLL' name 'ParDrvCommand'; //nolang

//function ParDrvCommand(Command:Word;var Params,Results):Word; cdecl;
//         external 'SBIG32M.DLL' index 1; //nolang

//function ParDrvCommand(Command : ShortInt;
//                       var Params,
//                       Results) : ShortInt; cdecl; far;
//                       external 'SBIG32M.DLL' index 1;

//function ParDrvCommand(Command : ShortInt;
//                       Params  : Pointer;
//                       Results : Pointer) : ShortInt; cdecl; far;
//                       external 'SBIG32M.DLL' index 1;

function GetErrorSbig(NoError:Word):string;
function OpenDriverSbig:Word;
function CloseDriverSbig:Word;
function OpenDeviceSbig(PortNumber:Byte):Word;
...
function GetCameraNameSbig(var NameOut:String):Word;
...
var
   p:Pointer;
begin
Result:=ParDrvCommand(CC_OPEN_DRIVER,p,p);
end;
...
var
   p:Pointer;
begin
Result:=ParDrvCommand(CC_CLOSE_DRIVER,p,p);
end;
...
Result:=ParDrvCommand(CC_OPEN_DEVICE,OpenDeviceParams,p);
end;
...
var
   p:Pointer;
begin
Result:=ParDrvCommand(CC_CLOSE_DEVICE,p,p);
end;
...
Result:=ParDrvCommand(CC_GET_DRIVER_INFO,GetDriverInfoParams,GetDriverInfoResults0);
...
Result:=ParDrvCommand(CC_START_EXPOSURE,StartExposureParams,p);
end;
...
Result:=ParDrvCommand(CC_END_EXPOSURE,EndExposureParams,p);
end;
...
Result:=ParDrvCommand(CC_READOUT_LINE,ReadoutLineParams,Line);
end;
...
ReadoutLineParams.PixelStart:=_PixelStart;
ReadoutLineParams.PixelLength:=_PixelLength;
Result:=ParDrvCommand(CC_READOUT_LINE,ReadoutLineParams,p);
end;
...
Result:=ParDrvCommand(CC_DUMP_LINES,DumpLinesParams,p);
end;
...
Result:=ParDrvCommand(CC_END_READOUT,EndReadoutParams,p);
end;
...
DTAmb      :Double= 45.0;
       ...
       ParDrvCommandSbig(CC_SET_TEMPERATURE_REGULATION,SetTemperatureRegulationParams,null);}
...
Result:=ParDrvCommand(CC_SET_TEMPERATURE_REGULATION,SetTemperatureRegulationParams,p);
end;
...
begin
Temp:=-9999;
Result:=ParDrvCommand(CC_QUERY_TEMPERATURE_STATUS,p,QueryTemperatureStatusResults);
if Result=0 then
   begin
...
Result:=ParDrvCommand(CC_ACTIVATE_RELAY,ActivateRelayParams,p);
end;
...
Result:=ParDrvCommand(CC_PULSE_OUT,PulseOutParams,p);
Sleep(3000);
end;
...
Result:=ParDrvCommand(CC_GET_CCD_INFO,GetCCDInfoParams,GetCCDInfoResults0);
i:=1;
