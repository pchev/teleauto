...
var
   ...
   Error:Word;
begin
Error:=GetCameraWidthSbig(Value);
if Error<>0 then
   begin
   WriteSpy('GetCameraWidthSbig : '+GetErrorSbig(Error));
   Exit;
   end;
Result:=Value;
end;
...
var
   ...
   Error:Word;
begin
Error:=GetCameraHeightSbig(Value);
if Error<>0 then
   begin
   WriteSpy('GetCameraHeightSbig : '+GetErrorSbig(Error));
   Exit;
   end;
Result:=Value;
end;
...
WriteSpy('OpenDeviceSbig');
Error:=OpenDeviceSbigUSB;
if Error<>0 then
   begin
...
if Error<>0 then
   begin
   WriteSpy('StartExposureSbig : '+GetErrorSbig(Error));
   Result:=False;
   Exit;
   end;
...
if Error<>0 then WriteSpy('SetTempRegulOnSbig : '+GetErrorSbig(Error));
end;
...
var
   ...
   Error:Word;
begin
Error:=GetPixelWidthSbig(Value);
if Error<>0 then
   begin
   WriteSpy('GetPixelWidth : '+GetErrorSbig(Error));
   Exit;
   end;
...
var
   ...
   Error:Word;
begin
Error:=GetPixelHeightSbig(Value);
if Error<>0 then
   begin
   WriteSpy('GetPixelHeight : '+GetErrorSbig(Error));
   Exit;
   end;
