...
// webcam variables
  FrIma : Tbitmap;
  FrInf : TBitmapInfo;
  LPstatus,LPrate,LPdelay,nbrima,vestaquality : integer;
  Sx, Sy, webcam_sx, webcam_sy : integer;
  Multithread, webcamcolor, LockClose : boolean;
  LPValDebutPose,LPValFinPose,LPValAmpOn,LPValAmpOff,LPval : byte;
  debutpose,debutdelay,dureedelay : Tdatetime;
  UToffset : double;
  appdir : string;
...
begin
Result:=pop_vestasc.VideoCap1.DriverOpen;
end;
...
begin
LockClose:=true;
if pop_VestaSC=nil then pop_VestaSC:=Tpop_VestaSC.Create(nil);
if FrIma=nil then FrIma:=TBitmap.create;
if not pop_VestaSC.videocap1.driveropen then pop_VestaSC.SelectDriver1Click(pop_VestaSC);
pop_VestaSC.Show;
end;
...
begin
pop_VestaSC.VideoCap1.DriverOpen:=false;
...
LPstatus:=0;
pop_vestasc.Videocap1.PreviewRate:=1;
pop_vestasc.VideoCap1.VideoPreview:=pop_vestasc.Preview1.checked;
...
//////////////////////////////////////////
procedure ExpTimer;
var elapse : tdatetime;
begin
with pop_vestaSC do begin
case LPstatus of
2:begin;
    dureedelay:=LPdelay/1000/3600/24;
    LPstatus:=3;
    LPVal:=LPValFinPose;
    PortOut(Adress,LPVal);
    debutdelay:=now;
...
3..4:begin
    elapse:=now-debutdelay;
    if elapse>=dureedelay then begin
       // l'image est prete
       LPstatus:=5;
    end;
  end;
...
case LPstatus of
    ...
    3 : begin
         Lpstatus:=4;
         FrIma.Width := pop_VestaSC.videocap1.CapWidth;
         FrIma.Height := pop_VestaSC.videocap1.CapHeight;
...
procedure Tpop_VestaSC.SelectDriver1Click(Sender: TObject);
...
if AutoconnectOn then begin
 ...
 pop_WebcamSelect:=Tpop_WebcamSelect.Create(pop_VestaSC);
...
procedure Tpop_VestaSC.VideoSource1Click(Sender: TObject);
begin
 VideoCap1.DlgVSource;
 videocap1.GetVideoFormat(VestaQuality,false);
end;

procedure Tpop_VestaSC.VideoFormat1Click(Sender: TObject);
begin
...
procedure Tpop_VestaSC.FormCreate(Sender: TObject);
begin
decimalseparator:='.';
...
procedure Tpop_VestaSC.FormDestroy(Sender: TObject);
begin
Frima.free;
end;

procedure Tpop_VestaSC.WMMove(var Message: TWMMove);
begin
  inherited;
  // after move of form
...
procedure Tpop_VestaSC.WMSize(var Message: TWMSize);
begin
  inherited;
  // after size
  if LockClose and (message.SizeType=SIZE_MINIMIZED) then pop_VestaSC.windowstate:=wsNormal;
end;

procedure Tpop_VestaSC.Color1Click(Sender: TObject);
begin
Color1.Checked:= not Color1.Checked;
webcamcolor:=Color1.Checked;
end;

procedure Tpop_VestaSC.FormClose(Sender: TObject; var Action: TCloseAction);
var ini:TInifile;
begin
If LockClose then begin
   Action:=caNone;
   pop_VestaSC.Height:=80;
   pop_VestaSC.Width:=100;
end else begin
   Action:=caHide;
   chdir(appdir);
   ini:= TInifile.Create('.\vestasc.ini'); //nolang
   ...
   ini.writeinteger('Options','Delay',LPdelay); //nolang
   ini.writeinteger('Options','Rate',LPrate); //nolang
   ini.writeinteger('Options','BeginExposure',LPValDebutPose); //nolang
   ini.writeinteger('Options','EndExposure',LPValFinPose); //nolang
   ini.writeinteger('Options','AmpliOn',LPValAmpOn); //nolang
   ini.writeinteger('Options','AmpliOff',LPValAmpOff); //nolang
   ini.writeinteger('Options','Top',pop_VestaSC.Top); //nolang
   ini.writeinteger('Options','Left',pop_VestaSC.Left); //nolang
   ini.writeinteger('Options','Width',pop_VestaSC.Width); //nolang
   ini.writeinteger('Options','Height',pop_VestaSC.Height); //nolang
   ini.free;
end;
end;

procedure Tpop_VestaSC.AutoConnect1Click(Sender: TObject);
begin
AutoConnect1.Checked:= not AutoConnect1.Checked;
AutoconnectOn:=AutoConnect1.Checked;
end;

procedure Tpop_VestaSC.Preview1Click(Sender: TObject);
begin
...
procedure Tpop_VestaSC.FormCanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
...
procedure Tpop_VestaSC.FormShow(Sender: TObject);
begin
pop_VestaSC.Top:=VestaSCTop;
pop_VestaSC.Left:=VestaSCLeft;
pop_VestaSC.Width:=VestaSCWidth;
pop_VestaSC.Height:=VestaSCHeight;
end;
...
LPdelay:=ini.readinteger('Options','Delay',50); //nolang
LPrate:=ini.readinteger('Options','Rate',100); //nolang
LPValDebutPose:=ini.readinteger('Options','BeginExposure',4); //nolang
LPValFinPose:=ini.readinteger('Options','EndExposure',15); //nolang
LPValAmpOn:=ini.readinteger('Options','AmpliOn',4); //nolang
LPValAmpOff:=ini.readinteger('Options','AmpliOff',11); //nolang
