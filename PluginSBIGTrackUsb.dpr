...
var
   ...
   Error:Word;
begin
Error:=GetCameraTrackNameSbig(NameCamera);
if Error<>0 then
   begin
   WriteSpy('GetCameraTrackNameSbig : '+GetErrorSbig(Error));
   Exit;
...
if Error<>0 then
   begin
   WriteSpy('GetPixelTrackWidth : '+GetErrorSbig(Error));
   Exit;
   end;
...
if Error<>0 then
   begin
   WriteSpy('GetPixelTrackHeight : '+GetErrorSbig(Error));
   Exit;
   end;
